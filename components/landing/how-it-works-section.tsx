    ...
    >
      <div className="max-w-[1440px] mx-auto px-4 sm:px-8 lg:px-12 text-center">
        ...
        >
          ...
          {[
            { step: '1', title: 'Ask Your Question', desc: 'Type your legal question in plain language.', icon: <MessageSquare className="w-6 h-6 sm:w-8 sm:h-8" /> },
            { step: '2', title: 'AI Analysis', desc: 'Our AI searches through Philippine legal codes.', icon: <Search className="w-6 h-6 sm:w-8 sm:h-8" /> },
            { step: '3', title: 'Receive Guidance', desc: 'Receive cited legal information with references.', icon: <FileText className="w-6 h-6 sm:w-8 sm:h-8" /> },
            { step: '4', title: 'Take Action', desc: 'Use insights to make informed decisions.', icon: <ArrowRight className="w-6 h-6 sm:w-8 sm:h-8" /> },
          ].map((item) => (
            <motion.div
              key={item.step}
            ...
            >
              ...
              >
                <div className="text-gray-500 mb-4 sm:mb-8 transition-transform group-hover:scale-110 group-hover:text-[#e9c176]">{item.icon}</div>
