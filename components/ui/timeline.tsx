...
export const Timeline = ({ data }: { data: TimelineData[] }) => {
  ...
  const updateStatus = (index: number, newStatus: string) => {
    setLocalData(prev => {
      ...
      if (newStatus === 'completed' && (!updatedItem.date || updatedItem.date.trim() === '')) {
  ...
  return (
    <div className="w-full font-sans" ref={containerRef}>
      ...
      <div ref={ref} className="relative border-l-2 border-[#8B4564]/20 ml-[4.5rem] md:ml-[6rem] pb-8">
        <div className="space-y-8">
          {(() => {
            ...
            return localData.map((item, i) => {
              ...
              if (item.requires_previous && i > 0 && computedStatus === 'pending') {
                const prevStatus = localData[i - 1].status?.toLowerCase() || 'pending';
              ...
              return (
                <div key={i} className={`relative pl-6 md:pl-8 flex w-full ${openDropdown === i || lockedAlert === i ? 'z-[100]' : 'z-10'}`}>
                  {showDate && (
                  ...
                  {/* Outer circle dot */}
                  <div className={`absolute -left-[9px] top-1.5 w-4 h-4 rounded-full border-2 border-[#1A1A1A] z-20 flex items-center justify-center ${
                                isCompleted ? 'bg-[#10B981]' : 
                                isLocked ? 'bg-[#2A2A2A] border-gray-600' : 'bg-[#8B4564]/50'
                   }`}>
                     {isLocked && <svg className="w-2 h-2 text-gray-500" fill="currentColor" viewBox="0 0 20 20"><path d="M10 2a4 4 0 00-4 4v2H5a2 2 0 00-2 2v7a2 2 0 002 2h10a2 2 0 002-2v-7a2 2 0 00-2-2h-1V6a4 4 0 00-4-4zm2 6H8V6a2 2 0 114 0v2z"/></svg>}
                  </div>
                  
                  {/* Card content */}
                  <div className={`flex-1 p-5 rounded-xl shadow-none border ${isLocked ? 'bg-[#1A1A1A] border-transparent opacity-60' : 'bg-[#2A2A2A] border-white/5'}`}>
                    <div className="flex justify-between items-start mb-2 relative">
                      <h3 className={`font-semibold text-base md:text-lg ${isLocked ? 'text-gray-500' : 'text-white'}`}>{item.title}</h3>
                      ...
                      {isCompleted ? (
                        ...
                        <div className="relative">
                          ...
                          >
                            <svg className="w-3 h-3" fill="currentColor" viewBox="0 0 20 20"><path d="M10 2a4 4 0 00-4 4v2H5a2 2 0 00-2 2v7a2 2 0 002 2h10a2 2 0 002-2v-7a2 2 0 00-2-2h-1V6a4 4 0 00-4-4zm2 6H8V6a2 2 0 114 0v2z"/></svg>
                            LOCKED
                          </button>
                          ...
                          {lockedAlert === i && (
                            <div className="absolute right-0 top-full mt-2 w-52 bg-[#2A1A1A] border border-red-500/30 rounded-xl p-3 shadow-2xl z-[100] animate-in fade-in zoom-in-95 duration-200">
                               <p className="text-[11px] leading-relaxed text-red-200 font-medium">
                                 Action strictly required: You must mark <strong className="text-red-400 font-bold">"{localData[i-1]?.title || 'the previous step'}"</strong> as completed before this unlocks.
                               </p>
                            </div>
                          )}
                        </div>
                        ...
                        <div className="relative">
                          ...
                          >
                            {item.status || 'PENDING'}
                            <svg className={`w-3 h-3 transition-transform duration-200 ${openDropdown === i ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg>
                          </button>
                          ...
                          {openDropdown === i && (
                            <div className="absolute right-0 top-full mt-2 w-32 bg-[#1A1A1A] border border-white/10 rounded-xl shadow-2xl overflow-hidden z-[100] animate-in fade-in zoom-in-95 duration-200">
                              <button 
                                onClick={() => { updateStatus(i, 'pending'); setOpenDropdown(null); }}
                              ...
                              <button 
                                onClick={() => { updateStatus(i, 'completed'); setOpenDropdown(null); }}
                    ...
                    <p className={`text-sm leading-relaxed mt-2 ${isLocked ? 'text-gray-600' : 'text-gray-300'}`}>{item.description}</p>
                  </div>
                </div>
              );
            });
