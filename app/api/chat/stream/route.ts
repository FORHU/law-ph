...
export async function POST(request: NextRequest) {
    try {
        ...
        if (!session_id) {
            return new Response(
                JSON.stringify({
                    error: "Missing session_id. Initialize via GET /api/chat/session first.",
                }),
                {
                    status: 400,
                    headers: { "Content-Type": "application/json" },
        ...
        const apiUrl = (
            process.env.CHAT_WONDER_API_URL || "http://localhost:8000"
        ).replace(/\/+$/, "");
        const wsUrl = apiUrl
            .replace("http://", "ws://")
            .replace("https://", "wss://");
        ...
        const stream = new ReadableStream({
            start(controller) {
                ...
                ws.onopen = () => {
                    console.log(
                        "[chat/stream] WebSocket connected to chat-wonder",
                    );
                    ...
                    console.log(
                        "[chat/stream] → WS payload",
                        JSON.stringify({
                            ...payload,
                            user_input:
                                payload.user_input.slice(0, 120) +
                                (payload.user_input.length > 120 ? "…" : ""),
                        }),
                    );
